defmodule QuickCrud.Testing do
  @moduledoc """
  Documentation for `QuickCrud.Testing`.
  ...
  defmacro list(config) do
    quote do
      test "list_#{unquote(config).plural_name}" do
        QuickCrud.Testing.check_list(unquote(config))
      end
    end
  end
  ...
  defmacro get(config) do
    quote do
      test "get_#{unquote(config).singular_name}" do
        QuickCrud.Testing.check_get(unquote(config))
      end
    end
  end
  ...
  defmacro new(config) do
    quote do
      test "new_#{unquote(config).singular_name}" do
        QuickCrud.Testing.check_new(unquote(config))
      end
    end
  end
  ...
  defmacro change(config) do
    quote do
      test "change_#{unquote(config).singular_name}" do
        QuickCrud.Testing.check_change(unquote(config))
      end
    end
  end
  ...
  defmacro create(config, attrs) do
    quote do
      test "create_#{unquote(config).singular_name}" do
        QuickCrud.Testing.check_create(unquote(config), unquote(attrs), fn _config -> %{} end)
      end
    end
  end
  ...
  defmacro create(config, attrs, extra_attrs_fun) do
    quote do
      test "create_#{unquote(config).singular_name}" do
        QuickCrud.Testing.check_create(unquote(config), unquote(attrs), unquote(extra_attrs_fun))
      end
    end
  end
  ...
  defmacro update(config, attrs) do
    quote do
      test "update_#{unquote(config).singular_name}" do
        QuickCrud.Testing.check_update(unquote(config), unquote(attrs), fn _config -> %{} end)
      end
    end
  end
  ...
  defmacro update(config, attrs, extra_attrs_fun) do
    quote do
      test "update_#{unquote(config).singular_name}" do
        QuickCrud.Testing.check_update(unquote(config), unquote(attrs), unquote(extra_attrs_fun))
      end
    end
  end
  ...
  defmacro delete(config) do
    quote do
      test "delete_#{unquote(config).singular_name}" do
        QuickCrud.Testing.check_delete(unquote(config))
      end
    end
  end
  ...
  defmacro paginate(config) do
    quote do
      test "paginate_#{unquote(config).singular_name}" do
        QuickCrud.Testing.check_paginate(unquote(config))
      end
    end
  end
